...
<?php
    ...
    // Truy vấn để lấy thông tin ban đầu từ cơ sở dữ liệu
    $result = mysqli_query($con,$query = "SELECT tsbn.maBN, tsbn.tenBN, tsbn.lydovaovien, tsbn.quatrinhbenhly, tsbn.tieusubenhnhan, tsbn.dacdiem, tsbn.giadinh, tt.trangThai 
    FROM tsbn, tt 
    WHERE tsbn.trangThai = tt.id");
?>
<head>
    <meta charset="UTF-8">
...
.them:hover {
        ...
        <div class="container-fluid">
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>Cập nhật thông tin khám bệnh </h2>
                            <!-- <a class="them"
 ...
 <form id="updateForm" action="./control/suatrangthai.php" method="POST">
    <!-- Form để cập nhật trạng thái bệnh nhân -->
    <div class="flex gap-2 thongke">
        <div>
            <select name="maBN" id="maBN" style="width: 200px;" class="border-[1px] border-[black]">
                ...
                <?php
                // Truy vấn để lấy danh sách mã bệnh nhân có trạng thái là 1
                $query = "SELECT tsbn.maBN FROM tsbn, tt WHERE tsbn.trangThai = tt.id and tsbn.trangThai=1";
        ...
        <div>
            <select name="trangThai" id="trangThai" style="width: 150px;" class="border-[1px] border-[black]">
                ...
                <?php
                // Truy vấn để lấy danh sách id của trạng thái
                $query = "SELECT id FROM tt";
                $results = mysqli_query($con, $query);
                if ($results) {
                    while ($data = mysqli_fetch_assoc($results)) {
        ...
        <div>
            <button type="submit" class="border-[1px] border-[#38A169] h-[30px] w-[90px] rounded text-[white] font-bold bg-[#38A169]">Cập Nhật</button>
                            ...
                            if (isset($_POST['maBN'])) {
                                ...
                                $query = "SELECT tsbn.maBN, tsbn.tenBN, tsbn.lydovaovien, tsbn.quatrinhbenhly, tsbn.tieusubenhnhan, tsbn.dacdiem, tsbn.giadinh, tt.trangThai 
                                FROM tsbn, tt 
                                WHERE tsbn.trangThai = tt.id and tsbn.maBN='$maBN'";
                                $result = mysqli_query($con, $query);
                            }
                            ?>
                            <!-- Hiển thị danh sách bảng cập nhật khám bệnh của bệnh nhân  -->
                            <?php if (isset($result)): ?>
                                <div class="table">
